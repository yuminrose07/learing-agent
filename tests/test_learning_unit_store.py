...
class TestForgeStateRoundTrip:
    ...
    def test_new_unit_defaults(self, file_store: FileStore):
        ...
        assert raw["orientation_context"] is None
    ...
    def test_forge_stage_round_trips_after_write(self, file_store: FileStore):
        ...
        assert reloaded.orientation_context is None

    def test_orientation_context_round_trips_after_write(self, file_store: FileStore):
        store = LearningUnitStore(file_store)
        unit = store.create(session_id="sess-orientation", objective_text="t")
        from learning_agent.ai.learning_unit import OrientationContext

        unit.orientation_context = OrientationContext(
            prompt_text="如果换成你解释这个目标，你会先抓哪条线索？",
            hook_kind="scenario",
            source="llm",
            source_seed_ref="objective:t",
        ...
        reloaded = fresh.get(unit.id)
        assert reloaded is not None
        assert reloaded.orientation_context is not None
        assert reloaded.orientation_context.source == "llm"
        assert reloaded.orientation_context.hook_kind == "scenario"
        assert reloaded.orientation_context.orientation_digest == "digest123"
    ...
    def test_legacy_1a_collision_without_orientation_loads(self, file_store: FileStore):
        legacy = {
            "id": "lu-legacy-collision",
            "session_id": "sess-legacy-collision",
            "objective": {
                "text": "旧卷已在 collision",
                "source": "ai_distilled",
                "source_ref": None,
        ...
        store = LearningUnitStore(file_store)
        reloaded = store.get("lu-legacy-collision")
        assert reloaded is not None
        assert reloaded.forge_stage == "collision"
        assert reloaded.orientation_context is None
