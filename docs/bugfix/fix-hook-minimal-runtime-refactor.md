...
在 `learning_agent/ai/models.py` 中新增以下模型：
...
`learning_agent/agent/hook_system.py` 从统一的泛型执行器重构为 typed dispatcher：
...
| # | 文件 | 改动说明 |
|---|------|---------|
| 1 | `learning_agent/ai/models.py` | 新增 5 个 Hook 的 typed 契约模型与公共枚举/上下文模型 |
| 2 | `learning_agent/models/__init__.py` | 导出新的 Hook typed 模型 |
| 3 | `learning_agent/agent/hook_system.py` | 重构为 typed dispatcher，新增五个 `run_*` 方法与固定 merge 规则 |
| 4 | `learning_agent/learning_agent/extension_manager.py` | `register_hook()` 改为接收 `HookName` |
...
| 8 | `learning_agent/learning_agent/extensions/built_in.py` | 收敛默认内置扩展清单，迁移 `core-fulltrace` 和观测类扩展到新 Hook |
