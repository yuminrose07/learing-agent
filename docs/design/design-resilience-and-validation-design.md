...
**位置**：`learning_agent/agent/tool_validator.py`（建议新建）
...
**位置**：`learning_agent/agent/tool_failure_tracker.py`（建议新建）或内嵌于 `SessionManager`
...
**文件**：`learning_agent/learning_agent/main.py` 或 Provider 组装处
...
- [ ] 新建 `learning_agent/agent/tool_validator.py`，实现 `ToolInputValidator`
- [ ] 新建 `learning_agent/agent/tool_failure_tracker.py`，实现 `ToolFailureTracker`
